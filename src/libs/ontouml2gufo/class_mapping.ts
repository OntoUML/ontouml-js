...
export function getModeGufoParentFromRelations(classElement: IClass): string {
  ...
  const relationStereotypes = relations.filter((relation: IRelation) => relation.stereotypes !== null)
                                       .map((relation: IRelation) => relation.stereotypes[0]);

  return relationStereotypes.includes(RelationStereotype.CHARACTERIZATION) &&
       relationStereotypes.includes(RelationStereotype.EXTERNAL_DEPENDENCE) ? 'gufo:ExtrinsicMode' : 'gufo:IntrinsicMode';
...
export function getGufoParentFromAllowed(classElement: IClass): string {
  ...
  if (allowed.length === 1){
    ...
    if(nature === 'mode')
      return getModeGufoParentFromRelations(classElement);

    if(nature === 'collective')
      return getCollectiveGufoParent(classElement);
  ...
  if (allowed.includes('abstract'))
    return 'gufo:Individual';

  if (allowed.includes('event'))
    return 'gufo:ConcreteIndividual';

  if ( !allowed.includes('collective') &&
...
export function getGufoParent(classElement: IClass): string {
  ...
  for (let parent of parents){
    if (parent.type !== OntoUMLType.CLASS_TYPE)
      continue;
