...
import { NormalRelationStereotypeMapping, InverseRelationStereotypeMapping, IgnoredGUFOInverseRelations } from './constants';
  ...
  for (const stereotype of Object.keys(InverseRelationStereotypeMapping)) {
    ...
    if (inverseProperty && isInverseEnabled) {
      ...
      if (properPartInverseList.includes(inverseProperty)) {
        quads.push(quad(namedNode(`:${inverseProperty}`), namedNode('rdfs:subPropertyOf'), namedNode(':hasProperPart')));
      }

      if (objectProperPartInverseList.includes(inverseProperty)) {
        quads.push(quad(namedNode(`:${inverseProperty}`), namedNode('rdfs:subPropertyOf'), namedNode(':hasObjectProperPart')));
      }
    }
  }
