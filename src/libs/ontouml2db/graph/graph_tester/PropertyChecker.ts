...
export class PropertyChecker{
    ...
    public check(node: INode): string{
        ...
        if( property == null )
        return "The '" + this.name + "' property was not found in '"+node.getName()+"' node.";

        if( this.nullable != property.isNullable() ){
            return "The annulability of '" + this.name + "' in '"+ node.getName() + "' table is diferente form the graph.";
        }
