...
export class TrackerContainer implements TrackerContainer {
  private sourceNode: Node; //Node to which the container belongs.
  private tracker: Map<string, Tracker>;
  ...
  addTracking(trackers: Tracker[]): void {
    trackers.forEach((value: Tracker) => {
    ...
    for (let nodeTracker of this.tracker.values()) {
      if (nodeTracker != null) {
        ...
        if (nodeTracker.getNode().getName() == this.sourceNode.getName()) {
          nodeTracker
            .getNode()
            .setTrackerField(this.sourceNode, property, value);
        }
      }
    }
  }
  ...
  setSourcePropertyLinkedAtNode(linkedNode: Node): void {
    for (let nodeTracker of this.tracker.values()) {
      ...
      if (nodeTracker.getNode().getName() == this.sourceNode.getName()) {
        nodeTracker
          .getNode()
          .setPropertyLinkedAtNode(this.sourceNode, linkedNode);
      }
    }
  }
