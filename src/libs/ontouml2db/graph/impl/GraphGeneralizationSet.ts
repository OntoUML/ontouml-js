...
export class GraphGeneralizationSet extends GraphAssociation implements IGraphGeneralizationSet{
    ...
    private getGeneraization(superNode: INode, subNode: INode): IGraphGeneralization{
        let i: number = 0;
        let generalizations = superNode.getGeneralizations();

        while (i < generalizations.length){
            if( generalizations[i].getGeneralizationNode().getId() == superNode.getId() &&
