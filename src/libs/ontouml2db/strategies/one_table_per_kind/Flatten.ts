...
export class Flatten {
  ...
  private static flattenGeneralization(
    ...
    generalization
      .getGeneralizationNode()
      .addSourceTrackedNode(generalization.getSpecializationNode());
    generalization
      .getSpecializationNode()
      .addTracking(generalization.getGeneralizationNode().getTrackers());
    generalization.getGeneralizationNode().removeSourceTracking();
  }

  private static flattenAssociation(
    flattenNode: INode,
    relation: IGraphRelation,
    graph: IGraph,
  ): void {
    for (let generalization of flattenNode.getGeneralizations()) {
      this.flattenAssociationWith(
        flattenNode,
        generalization.getSpecializationNode(),
        relation,
        graph,
      );
    }
  ...
  private static flattenAssociationWith(
    flattenNode: INode,
    toNode: INode,
    relation: IGraphRelation,
    graph: IGraph,
