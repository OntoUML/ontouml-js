 ...
 export class SolvesForeignKey{
    ...
    public static solves( graph: IGraph ): void {
        for (let relation of graph.getAssociations() as IGraphRelation[]) {
			if(     (relation.getSourceCardinality() == Cardinality.C0_1 || 
				...
				this.propagateKey(
                    relation.getSourceNode(), relation.getSourceCardinality(), 
					relation.getTargetNode(), relation);
			}
			else if( 	(relation.getTargetCardinality() == Cardinality.C0_1 || 
				...
				this.propagateKey(
                    relation.getTargetNode(), relation.getTargetCardinality(), 
                    relation.getSourceNode(), relation);
			}
		}
	}
	
	private static propagateKey(from: INode, cardinalityFrom: Cardinality, to: INode, relation: IGraphRelation): void {
    ...
    private static getNewFKName( prop: INodeProperty, relation: IGraphRelation): string {
		let result: string;
