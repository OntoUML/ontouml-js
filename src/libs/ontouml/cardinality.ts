...
export class Cardinality {
  ...
  getCardinalityBoundsAsNumbers(): { lowerBound: number; upperBound: number } {
    ...
    return bounds && { lowerBound: parseInt(bounds.lowerBound), upperBound: parseInt(bounds.upperBound) };
  }
  ...
  isValid(): boolean {
    const regex = /(\d+\.\.)?(\d+|\*)/;
    const bounds = this.getCardinalityBounds();
    return this.value && regex.test(this.value) && bounds.upperBound >= bounds.lowerBound;
  }
