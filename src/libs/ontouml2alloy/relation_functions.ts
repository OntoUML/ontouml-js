...
export function transformRelation(transformer: Ontouml2Alloy, relation: Relation) {
  ...
  if (
    relation.isPartWholeRelation() ||
    relation.hasComponentOfStereotype() ||
    relation.hasMemberOfStereotype() ||
    relation.hasSubCollectionOfStereotype() ||
    relation.hasSubQuantityOfStereotype()
  ) {
...
function transformOrderedRelation(transformer: Ontouml2Alloy, relation: Relation) {
  ...
  transformer.addWorldFieldDeclaration(relationName + ': set ' + sourceName + ' set -> set Int set -> set ' + targetName);
...
function transformGeneralRelation(transformer: Ontouml2Alloy, relation: Relation) {
  ...
  transformer.addWorldFieldDeclaration(
    (relationName + ': set ' + sourceName + ' ' + sourceCardinality + ' -> ' + targetCardinality + ' ' + targetName).replace(
      /\s{2,}/g,
      ' '
    )
  );
...
function transformMaterialRelation(transformer: Ontouml2Alloy, relation: Relation) {
  for (const rel of transformer.model.getAllRelations()) {
    if (
      rel.hasDerivationStereotype() &&
      rel.isDerivation() &&
      rel.getDerivingRelation() === relation &&
      ...
      transformer.addWorldFieldDeclaration(materialName + ': set ' + sourceName + ' -> ' + relatorName + ' -> ' + targetName);
      return;
    }
  }
...
function transformDerivationRelation(transformer: Ontouml2Alloy, relation: Relation) {
  ...
  if (
    relation.getDerivingRelationStereotype() === RelationStereotype.MATERIAL &&
...
function transformDatatypeRelation(transformer: Ontouml2Alloy, relation: Relation) {
  ...
  transformer.addFact('fact datatypesMultiplicity {\n' + '        ' + sourceFact + '\n' + '        ' + targetFact + '\n' + '}');
}

export function transformCharacterizationConstraint(transformer: Ontouml2Alloy) {
  let characterizations = transformer.model
    .getAllRelationsByStereotype(RelationStereotype.CHARACTERIZATION)
    .map(characterization => {
      return 'w.' + getNormalizedName(transformer, characterization);
    });

  if (characterizations.length) {
    let intrinsicMoments = [
      ...new Set([
        ...transformer.model.getClassesRestrictedToIntrinsicMode(),
        ...transformer.model.getClassesRestrictedToQuality()
      ])
    ...
    if (intrinsicMoments.length) {
      transformer.addFact(
        'fact acyclicCharacterizations {\n' +
          '        all w: World | acyclic[(' +
          characterizations.join('+') +
          '),(' +
          intrinsicMoments.join('+') +
          ')]\n' +
          '}'
      );
    }
