...
export function getCustomCardinality(cardinality: Cardinality) {
  ...
  if (!cardinality.isLowerBoundValid()) {
    // ! is being used because this method is returning the opposite of what it should
  ...
  if (material.hasMaterialStereotype()) {
    for (const rel of relations) {
      if (
        ...
        rel.isDerivation() &&
        rel.getDerivingRelation() === material &&
...
export function holdsBetweenDatatypes(relation: Relation) {
  if (
    relation.getSourceStereotype() === ClassStereotype.DATATYPE &&
