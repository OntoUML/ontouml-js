...
describe('Basic viewpoint example', () => {
  ...
  describe('Test if there is no nonsortal in model', () => {
    const project = new Project();
    const model = project.createModel();
    ...
    let diagrams: Diagram[] = new ViewpointExtractor(project).buildAll();
    let diagram: Diagram = diagrams.find(d => d.getName() === 'Non Sortal View');

    it('Test if Non Sortal is in model', () => expect(diagram.getClassViews()).toHaveLength(0));
  });
});
