...
describe('Basic viewpoint example', () => {
  ...
  describe('Test if there is no relator in model', () => {
    const project = new Project();
    const model = project.createModel();
    ...
    let diagrams: Diagram[] = new ViewpointExtractor(project).buildAll();
    let diagram: Diagram = diagrams.find(d => d.getName() === 'Relator View');

    it('Test if Relator is in model', () => expect(diagram.getClassViews()).toHaveLength(0));
  });
