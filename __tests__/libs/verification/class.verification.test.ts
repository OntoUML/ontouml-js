...
describe('Class constraints verification - old set of tests', () => {
  ...
  it(`Checks code ${VerificationIssueCode.class_missing_nature_restrictions}`, () => {
    ...
    const personIssue: VerificationIssue = issues.find(
      (issue: VerificationIssue) =>
        issue.source.id === person.id && issue.code === VerificationIssueCode.class_missing_nature_restrictions
    );
    const organizationIssue: VerificationIssue = issues.find(
      (issue: VerificationIssue) =>
        issue.source.id === organization.id && issue.code === VerificationIssueCode.class_missing_nature_restrictions
    );
    const legalEntityIssue: VerificationIssue = issues.find(
      (issue: VerificationIssue) =>
        issue.source.id === legalEntity.id && issue.code === VerificationIssueCode.class_missing_nature_restrictions
    );
