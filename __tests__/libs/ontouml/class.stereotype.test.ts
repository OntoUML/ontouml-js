...
describe('Test Class stereotype-related query methods', () => {
  ...
  describe(`Test ${Class.prototype.hasStereotype.name}()`, () => {
    it('Test classWithoutStereotypes', () => {
      const clazz = model.createClass();
      expect(clazz.hasStereotype(stereotypeUtils.ClassStereotypes)).toBe(false);
    });

    it('Test classWithUniqueStereotype', () => {
      const clazz = model.createKind();
      expect(clazz.hasStereotype(ClassStereotype.KIND)).toBe(true);
    });
  });
  ...
  describe(`Test ${Class.prototype.isAbstractStereotype.name}()`, () => {
    it('Test abstract', () => {
      const abstract = model.createAbstract();
      expect(abstract.isAbstractStereotype()).toBe(true);
    });
    it('Test anotherClass', () => {
      const clazz = model.createDatatype();
      expect(clazz.isAbstractStereotype()).toBe(false);
    });
    it('Test classWithoutStereotypes', () => {
      const classWithoutStereotypes = model.createClass();
      expect(classWithoutStereotypes.isAbstractStereotype()).toBe(false);
    });
  });
  ...
  describe(`Test ${Class.prototype.isEnumeration.name}()`, () => {
    it('Test enumeration', () => {
      const enumeration = model.createEnumeration();
      expect(enumeration.isEnumeration()).toBe(true);
    });
    it('Test anotherClass', () => {
      const clazz = model.createType();
      expect(clazz.isEnumeration()).toBe(false);
    });
    it('Test classWithoutStereotypes', () => {
      const classWithoutStereotypes = model.createClass();
      expect(classWithoutStereotypes.isEnumeration()).toBe(false);
    });
  });
  ...
  describe(`Test ${Class.prototype.isUltimateSortal.name}()`, () => {
    it('Test quality', () => {
      const quality = model.createQuality();
      expect(quality.isUltimateSortal()).toBe(true);
    });

    it('Test anotherClass', () => {
      const clazz = model.createHistoricalRole();
      expect(clazz.isUltimateSortal()).toBe(false);
    });

    it('Test classWithoutStereotypes', () => {
      const classWithoutStereotypes = model.createClass();
      expect(classWithoutStereotypes.isUltimateSortal()).toBe(false);
    });
  });
