...
describe('relation_functions', () => {
  ...
  it('should generate a derivation fact for a derivation relation from material relation', () => {
    ...
    expect(result).toContain(
      generateFact('derivation', [
        'all w: World, x: w.Organization, y: w.Employee, r: w.Employment | ',
        '    x -> r -> y in w.hires iff x in r.(w.relation) and y in r.(w.relation1)'
      ])
    );
  });
  ...
  it('should generate relator rule for 2 mediation relations', () => {
    ...
    expect(result).toContain(
      generateFact('relatorConstraint', ['all w: World, x: w.Enrollment | #(Student1[x,w]+University1[x,w])>=2'])
    );
  });
