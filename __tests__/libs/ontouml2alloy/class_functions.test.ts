...
describe('Class Functions', () => {
  describe('transformClass function', () => {
    ...
    it('should transform <<datatype>> class with attributes (complex datatype)', () => {
      ...
      const factLines = ['Datatype = Number+Date', 'disj[Number,Date]'];
    ...
    describe('Disjointness Constraints', () => {
      it('should emit a single disjoint constraint for ultimate sortals', () => {
        model.createKind('Person');
        model.createKind('Animal');
        model.createRelator('Marriage');
        const result = generateAlloy(model);

        const disjConstraints = result.match(/disj\[[^\]]+\]/g);
        expect(disjConstraints).toEqual(['disj[Animal,Marriage,Person]']);
      });

      it('should make each ultimate sortal disjoint from all other ultimate sortals (all stereotypes)', () => {
        model.createKind('Person');
        model.createCollective('Team', false);
        model.createQuantity('Water');
        model.createRelator('Bond');
        model.createQuality('Red');
        model.createIntrinsicMode('Skill');
        const result = generateAlloy(model);
        expect(result).toContain('disj[Bond,Person,Red,Skill,Team,Water]');
      });

      it('should make two ultimate sortals of the same stereotype disjoint', () => {
        model.createKind('Person');
        model.createKind('Animal');
        const result = generateAlloy(model);

        expect(result).toContain('disj[Animal,Person]');
      });

      it('should make ultimate sortals disjoint also when they specialize a shared category', () => {
        const agent = model.createCategory('Agent');
        const person = model.createKind('Person');
        const animal = model.createKind('Animal');
        model.createGeneralization(agent, person);
        model.createGeneralization(agent, animal);
        const result = generateAlloy(model);

        expect(result).toContain('disj[Animal,Person]');
      });

      it('should NOT make two categories disjoint', () => {
        model.createCategory('Agent');
        model.createCategory('PhysicalObject');
        const result = generateAlloy(model);
        expect(result).not.toContain('disj[Agent');
        expect(result).not.toContain('disj[PhysicalObject');
      });
    });
  });
});
