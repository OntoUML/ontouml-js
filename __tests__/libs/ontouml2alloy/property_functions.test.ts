...
import {
  ...
  generateFact,
  generateFun,
  generateWorldFieldForClass,
  generateWorldFieldForRelation,
  generateWorldFact
} from './helpers';
import {
  Class,
...
describe('property_functions', () => {
  ...
  describe('datatype properties', () => {
    ...
    it('should generate "datatypesMultiplicity fact for a relation between datatypes with default cardinality', () => {
      ...
      expect(result).toContain(
        generateFact('datatypesMultiplicity', ['all x: Date | #x.relation>=0', 'all x: String_class | #relation.x>=0'])
      );
    });
    ...
    it('should generate "datatypesMultiplicity fact for a relation between datatypes with custom cardinality', () => {
      ...
      expect(result).toContain(
        generateFact('datatypesMultiplicity', [
          'all x: Patient | #x.hasHealthRecord>=1',
          'all x: HealthRecord | #hasHealthRecord.x>=0'
        ])
      );
    });
  });
  ...
  describe('class attributes', () => {
    ...
    describe('ordered attributes', () => {
      it('should generate an "ordering" fact for an ordered attribute', () => {
        ...
        expect(result).toContain(
          generateFact('ordering', [
            'all w: World, x: w.Playlist | isSeq[x.(w.songs)]',
            'all w: World, x: w.Playlist, y: w.Playlist | lone x.((w.songs).y)'
          ])
        );
      });
      ...
      it('should transform an ordered attribute of a class with zero-to-one multiplicity', () => {
        ...
        expect(result).toContain(
          'fun nextMilestone1 [x: World.Project, w: World] : set Milestone {\n        x.(w.nextMilestone)\n}'
        );
      });
      ...
      it('should generate a "multiplicity" fact for an ordered attribute with custom cardinlaity', () => {
        ...
        expect(result).toContain(
          generateFact('multiplicity', ['all w: World, x: w.Playlist | #songs1[x,w]>=1 and #songs1[x,w]<=31'])
        );
      });
    });
    ...
    describe('unordered attributes', () => {
      ...
      it('should transform an unordered attribute with one-to-many multiplicity', () => {
        ...
        expect(result).toContain('fun members1 [x: World.Family, w: World] : set World.Member {\n        x.(w.members)\n}');
      });
      ...
      it('should transform an unordered attribute of a class with zero-to-one multiplicity', () => {
        ...
        expect(result).toContain('fun spouse1 [x: World.Person, w: World] : set World.Spouse {\n        x.(w.spouse)\n}');
      });
      ...
      it('should transform an unordered attribute of a class with one-to-one multiplicity', () => {
        ...
        expect(result).toContain('fun engine1 [x: World.Car, w: World] : set World.Engine {\n        x.(w.engine)\n}');
      });
      ...
      it('should generate a "multiplicity" fact for an unordered attribute with custom (1..3) multiplicity', () => {
        ...
        expect(result).toContain(
          generateFact('multiplicity', ['all w: World, x: w.Person | #homeAddress1[x,w]>=1 and #homeAddress1[x,w]<=3'])
        );
      });
    });

    describe('function return types', () => {
      describe('range is endurant class', () => {
        it('should use World.T in the fun return type when propertyType is an endurant class (unordered)', () => {
          const person = model.createKind('Person');
          const address = model.createKind('Address');
          person.createAttribute(address, 'homeAddress');

          const result = generateAlloy(model);

          expect(result).toContain(
            'fun homeAddress1 [x: World.Person, w: World] : set World.Address {\n        x.(w.homeAddress)\n}'
          );
          expect(result).not.toContain('fun homeAddress1 [x: World.Person, w: World] : set Address {');
        });

        it('should use World.T in the fun return type when propertyType is an endurant class (ordered)', () => {
          const itinerary = model.createKind('Itinerary');
          const stop = model.createKind('Stop');
          const orderedStops = itinerary.createAttribute(stop, 'orderedStops');
          orderedStops.isOrdered = true;

          const result = generateAlloy(model);

          expect(result).toContain(
            'fun orderedStops1 [x: World.Itinerary, w: World] : set World.Stop {\n        x.(w.orderedStops)\n}'
          );
          expect(result).not.toContain('fun orderedStops1 [x: World.Itinerary, w: World] : set Stop {');
        });
      });
      describe('range is value type', () => {
        it('should use a bare datatype name in the fun return type when propertyType is «datatype»', () => {
          const patient = model.createKind('Patient');
          const bloodPressure = model.createDatatype('BloodPressure');
          patient.createAttribute(bloodPressure, 'lastReading');

          const result = generateAlloy(model);

          expect(result).toContain(
            'fun lastReading1 [x: World.Patient, w: World] : set BloodPressure {\n        x.(w.lastReading)\n}'
          );
          expect(result).not.toContain('fun lastReading1 [x: World.Patient, w: World] : set World.BloodPressure {');
        });

        it('should use a bare enumeration name in the fun return type when propertyType is «enumeration»', () => {
          const flight = model.createKind('Flight');
          const gateDisplay = model.createEnumeration('GateDisplay');
          gateDisplay.createLiteral('OnTime');
          gateDisplay.createLiteral('Delayed');
          gateDisplay.createLiteral('Cancelled');
          flight.createAttribute(gateDisplay, 'boardingStatus');

          const result = generateAlloy(model);

          expect(result).toContain(
            'fun boardingStatus1 [x: World.Flight, w: World] : set GateDisplay {\n        x.(w.boardingStatus)\n}'
          );
          expect(result).not.toContain('fun boardingStatus1 [x: World.Flight, w: World] : set World.GateDisplay {');
        });
      });
    });
  });
  ...
  describe('relation ends', () => {
    describe('source end', () => {
      ...
      it('should transform a source end when opposite end is an enumeration', () => {
        const Flower = model.createKind('Flower');
        const FlowerColorValues = model.createEnumeration('FlowerColorValues');
        const hasColor = model.createBinaryRelation(Flower, FlowerColorValues, 'hasColor');
        hasColor.getSourceEnd().cardinality.setCardinalityFromNumbers(2, 2);

        const result = generateAlloy(model);

        expect(result).toContain('hasColor: set Flower -> set FlowerColorValues');
        expect(result).toContain('fun Flower1 [x: FlowerColorValues, w: World] : set World.Flower {\n        (w.hasColor).x\n}');
        expect(result).toContain(generateFact('multiplicity', ['all w: World, x: FlowerColorValues | #Flower1[x,w]=2']));
      });
      ...
      it('should transform a source end with specific range cardinality', () => {
        ...
        expect(result).toContain(
          generateFact('multiplicity', ['all w: World, x: w.Student | #Course1[x,w]>=1 and #Course1[x,w]<=50'])
        );
      });
    ...
    describe('target end', () => {
      ...
      it('should transform a target end when target is an enumeration', () => {
        const Flower = model.createKind('Flower');
        const FlowerColorValues = model.createEnumeration('FlowerColorValues');
        const hasColor = model.createBinaryRelation(Flower, FlowerColorValues, 'hasColor');
        hasColor.getTargetEnd().cardinality.setOneToOne();

        const result = generateAlloy(model);

        expect(result).toContain('hasColor: set Flower set -> one FlowerColorValues');
        expect(result).toContain(
          'fun FlowerColorValues1 [x: World.Flower, w: World] : set FlowerColorValues {\n        x.(w.hasColor)\n}'
        );
      });
      ...
      it('should transform a target end with specific range cardinality', () => {
        ...
        expect(result).toContain(
          generateFact('multiplicity', ['all w: World, x: w.Course | #Student1[x,w]>=1 and #Student1[x,w]<=50'])
        );
      });
